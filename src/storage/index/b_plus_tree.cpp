...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value) -> bool {
  if (IsEmpty()) {
    ...
    auto *root_leaf = reinterpret_cast<LeafPage *>(new_root_page->GetData());
  ...
  insert_into_parent = [&](Page *old_page, const KeyType &mid_key, Page *new_page) {
    ...
    int split_idx = parent->GetSize() / 2;
    KeyType parent_mid_key = parent->KeyAt(split_idx);
...
void BPLUSTREE_TYPE::Remove(const KeyType &key) {
  ...
  coalesce_or_redistribute = [&](Page *page) {
    ...
    if (node->IsLeafPage()) {
      auto *cur_leaf = reinterpret_cast<LeafPage *>(node);
      auto *sib_leaf = reinterpret_cast<LeafPage *>(sibling_node);

      if (cur_leaf->GetSize() + sib_leaf->GetSize() <= cur_leaf->GetMaxSize()) {
        LeafPage *left_leaf;
        LeafPage *right_leaf;
        Page *left_page;
        Page *right_page;
        ...
        if (sibling_is_left) {
          sib_leaf->MoveLastToFrontOf(cur_leaf);
          parent->SetKeyAt(parent_key_index, cur_leaf->KeyAt(0));
        } else {
          sib_leaf->MoveFirstToEndOf(cur_leaf);
          parent->SetKeyAt(parent_key_index, sib_leaf->KeyAt(0));
        }
        bpm_->UnpinPage(page->GetPageId(), true);
    ...
    auto *cur_internal = reinterpret_cast<InternalPage *>(node);
    auto *sib_internal = reinterpret_cast<InternalPage *>(sibling_node);

    if (cur_internal->GetSize() + sib_internal->GetSize() <= cur_internal->GetMaxSize()) {
      InternalPage *left_internal;
      InternalPage *right_internal;
      Page *left_page;
      Page *right_page;
      if (sibling_is_left) {
        left_internal = sib_internal;
        left_page = sibling_page;
        right_internal = cur_internal;
        right_page = page;
      } else {
        left_internal = cur_internal;
        left_page = page;
        right_internal = sib_internal;
        right_page = sibling_page;
      }

      right_internal->MoveAllTo(left_internal, parent_key);

      for (int i = 0; i < left_internal->GetSize(); ++i) {
        page_id_t child_id = left_internal->ValueAt(i);
        Page *child_page = bpm_->FetchPage(child_id);
        auto *child = reinterpret_cast<BPlusTreePage *>(child_page->GetData());
    ...
    if (sibling_is_left) {
      KeyType new_parent_key = sib_internal->KeyAt(sib_internal->GetSize() - 1);
      sib_internal->MoveLastToFrontOf(cur_internal, parent_key);
      page_id_t moved_child_id = cur_internal->ValueAt(0);
      ...
      KeyType new_parent_key = sib_internal->KeyAt(1);
      sib_internal->MoveFirstToEndOf(cur_internal, parent_key);
      page_id_t moved_child_id = cur_internal->ValueAt(cur_internal->GetSize() - 1);
      Page *moved_child_page = bpm_->FetchPage(moved_child_id);
      auto *moved_child = reinterpret_cast<BPlusTreePage *>(moved_child_page->GetData());
      moved_child->SetParentPageId(page->GetPageId());
      bpm_->UnpinPage(moved_child_id, true);
